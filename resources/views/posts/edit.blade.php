...
@section('content')
    @include('permissions.partials.header', ['title' => __('Edit Post')])
                        ...
                        <form method="post" action="{{ route('post.update', $post) }}" autocomplete="off">
                            ...
                            <div class="pl-lg-4">
                              ...
                              <div class="form-group{{ $errors->has('summary') ? ' has-danger' : '' }}">
                                  ...
                                  <textarea class="form-control form-control-alternative{{ $errors->has('summary') ? ' is-invalid' : '' }}" rows="4" placeholder="{{ __('Summary') }}" name="summary">{{ old('summary', $post->summary) }}</textarea>
                              ...
                              <div class="form-group{{ $errors->has('body') ? ' has-danger' : '' }}">
                                  ...
                                  <textarea id="body" class="form-control form-control-alternative{{ $errors->has('body') ? ' is-invalid' : '' }}" rows="4" placeholder="{{ __('Body') }}" name="body">{{ old('body', $post->body) }}</textarea>
                                  @if ($errors->has('body'))
                                      <span class="invalid-feedback" role="alert">
                              ...
                              <div class="form-group">
                                ...
                                <select class="form-control" name="live">
                                  <option value="0" {{ $post->live == 1 ? 'selected' : '' }}>False</option>
                                  <option value="1" {{ $post->live == 1 ? 'selected' : '' }}>True</option>
                                </select>
                              </div>
